...
const createCustomerHTML = (id, name = `Customer ${id}`, desc = "", time = "30:00") => `
...
const createCustomerElement = (id, name, desc, time) => {
    const div = document.createElement("div");
    div.className = "timer";
    div.id = `customer-${id}`;
    div.innerHTML = createCustomerHTML(id, name, desc, time);
...
window.addCustomer = async () => {
    ...
    customers[customerId] = { seconds: START_TIME, interval: null, created, name: name.trim(), timerId, paused: false };
    createCustomerElement(customerId, name.trim());
...
window.startCustomer = async id => {
    ...
    // Show notification and play sound when starting
...
onAuthStateChanged(auth, async user => {
    ...
    try {
        ...
        querySnapshot.forEach(doc => {
            ...
            createCustomerElement(id, data.customerName, data.description, formatTime(seconds));
            if (startTime) {
