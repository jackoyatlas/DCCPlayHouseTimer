...
async function loadPendingUsers() {
    ...
    users.forEach(user => {
        ...
        userDiv.className = "pending-user";
        userDiv.innerHTML = `
            <span>Name: ${user.fullName || 'N/A'} Email: ${user.email} Status: ${user.status || 'N/A'}</span>
...
function generateReportContent(timers, actions, startDate, endDate) {
    ...
    timers.forEach(timer => {
        ...
        const timerActions = actionsCountByTimer[timer.id] || { pause: 0, reset: 0 };
        ...
        // Calculate actual time (elapsed time)
        const actualTime = timer.time !== undefined ? formatTime(START_TIME - timer.time) : "N/A";
